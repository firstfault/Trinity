...
import me.f1nal.trinity.decompiler.struct.attr.StructEnclosingMethodAttribute;
...
public class NestedClassProcessor {
  public void processClass(ClassesProcessor.ClassNode root, ClassesProcessor.ClassNode node) {
    ...
    for (ClassesProcessor.ClassNode child : node.nested) {
      if (child.type == ClassesProcessor.ClassNode.CLASS_LAMBDA) {
        ...
        if (child.type == ClassesProcessor.ClassNode.CLASS_LOCAL && child.enclosingMethod != null) {
  ...
  private static void insertLocalVars(ClassesProcessor.ClassNode parent, ClassesProcessor.ClassNode child) {
