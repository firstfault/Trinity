...
package me.f1nal.trinity.decompiler.modules.decompiler.deobfuscator;
...
public final class IrreducibleCFGDeobfuscator {
  ...
  public static boolean isStatementIrreducible(Statement statement) {
    ...
    for (Statement stat : statement.getStats()) {
      if (!stat.getSuccessorEdges(StatEdge.EdgeType.EXCEPTION).isEmpty()) {
        return false;
      }
    ...
    for (Statement stat : statement.getStats()) {
      ...
      for (Statement succ : stat.getNeighbours(StatEdge.EdgeType.REGULAR, StatEdge.EdgeDirection.FORWARD)) {
  ...
  private static Statement getCandidateForSplitting(Statement statement) {
    ...
    for (Statement stat : statement.getStats()) {
      ...
      Set<Statement> setPreds = stat.getNeighboursSet(StatEdge.EdgeType.REGULAR, StatEdge.EdgeDirection.BACKWARD);

      if (setPreds.size() > 1) {
        int succCount = stat.getNeighboursSet(StatEdge.EdgeType.REGULAR, StatEdge.EdgeDirection.FORWARD).size();
  ...
  public static boolean splitIrreducibleNode(Statement statement) {
    ...
    StatEdge enteredge = splitnode.getPredecessorEdges(StatEdge.EdgeType.REGULAR).iterator().next();
    ...
    // switch input edges
    for (StatEdge prededge : splitnode.getPredecessorEdges(StatEdge.EdgeType.DIRECT_ALL)) {
      if (prededge.getSource() == enteredge.getSource() ||
          prededge.closure == enteredge.getSource()) {
        splitnode.removePredecessor(prededge);
        prededge.getSource().changeEdgeNode(StatEdge.EdgeDirection.FORWARD, prededge, splitcopy);
        splitcopy.addPredecessor(prededge);
      }
    }

    // connect successors
    for (StatEdge succ : splitnode.getSuccessorEdges(StatEdge.EdgeType.DIRECT_ALL)) {
  ...
  private static Statement copyStatement(Statement from, Statement to, HashMap<Statement, Statement> mapAltToCopies) {
    ...
    for (int i = 0; i < from.getStats().size(); i++) {
      ...
      for (StatEdge edgeold : stold.getSuccessorEdges(StatEdge.EdgeType.DIRECT_ALL)) {
