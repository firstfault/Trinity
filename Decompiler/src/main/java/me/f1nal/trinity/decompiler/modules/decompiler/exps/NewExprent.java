...
import me.f1nal.trinity.decompiler.struct.gen.generics.GenericClassDescriptor;
...
public class NewExprent extends Exprent {
  ...
  public NewExprent(VarType newType, List<Exprent> lstDims, Set<Integer> bytecodeOffsets) {
    ...
    if (newType.getType() == CodeConstants.TYPE_OBJECT && newType.getArrayDim() == 0) {
      ClassesProcessor.ClassNode node = DecompilerContext.getClassProcessor().getMapRootClasses().get(newType.getValue());
      if (node != null && (node.type == ClassesProcessor.ClassNode.CLASS_ANONYMOUS || node.type == ClassesProcessor.ClassNode.CLASS_LAMBDA)) {
        anonymous = true;
  ...
  public TextBuffer toJava(int indent, BytecodeMappingTracer tracer) {
    ...
    if (anonymous) {
      ClassesProcessor.ClassNode child = DecompilerContext.getClassProcessor().getMapRootClasses().get(newType.getValue());
      ...
      if (!enumConst && (!lambda || DecompilerContext.getOption(IFernflowerPreferences.LAMBDA_TO_ANONYMOUS_CLASS))) {
        ...
        buf.append(OutputMemberSerializer.serializeTags(new KeywordOutputMember("new ".length())));
        buf.append("new ");
      ...
      if (!enumConst) {
        ...
        if (enclosing != null) {
          ClassesProcessor.ClassNode newNode = DecompilerContext.getClassProcessor().getMapRootClasses().get(newType.getValue());
          if (newNode != null) {
  ...
  private static String getQualifiedNewInstance(String classname, List<Exprent> lstParams, int indent, BytecodeMappingTracer tracer) {
    ...
    if (node != null && node.type != ClassesProcessor.ClassNode.CLASS_ROOT && node.type != ClassesProcessor.ClassNode.CLASS_LOCAL
      ...
      if (!lstParams.isEmpty()) {
        ...
        if (enclosing.type == Exprent.EXPRENT_VAR) {
          ...
          StructClass current_class = ((ClassesProcessor.ClassNode)DecompilerContext.getProperty(DecompilerContext.CURRENT_CLASS_NODE)).classStruct;
