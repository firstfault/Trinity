...
import me.f1nal.trinity.decompiler.modules.decompiler.sforms.DirectNode;
import me.f1nal.trinity.decompiler.modules.decompiler.sforms.FlattenStatementsHelper;
import me.f1nal.trinity.decompiler.modules.decompiler.sforms.FlattenStatementsHelper.FinallyPathWrapper;
...
public class ExprProcessor implements CodeConstants {
  ...
  private final int methodIdHashcode;

  public ExprProcessor(MethodDescriptor md, VarProcessor varProc, int methodIdHashcode) {
    methodDescriptor = md;
    varProcessor = varProc;
    this.methodIdHashcode = methodIdHashcode;
  }
...
//      if (instr.offsetFromMethodStart != -1) {
//        exprList.add(new TrinityBytecodeMarkerExprent(instr, methodIdHashcode));
//      }
  ...
  public static String getTypeName(VarType type, boolean getShort, List<TypeAnnotationWriteHelper> typeAnnWriteHelpers) {
    ...
    if (tp <= CodeConstants.TYPE_BOOLEAN) {
      ...
      ret = OutputMemberSerializer.tag(ret, l -> new ClassOutputMember(l, type.getValue()));
  ...
  public static String getCastTypeNameWithoutTags(VarType type, List<TypeAnnotationWriteHelper> typePathWriteHelper) {
  ...
  public static TextBuffer jmpWrapper(Statement stat, int indent, boolean semicolon, BytecodeMappingTracer tracer) {
    ...
    List<StatEdge> lstSuccs = stat.getSuccessorEdges(StatEdge.EdgeType.DIRECT_ALL);
    if (lstSuccs.size() == 1) {
      StatEdge edge = lstSuccs.get(0);
      if (edge.getType() != StatEdge.EdgeType.REGULAR && edge.explicit && edge.getDestination().type != StatementType.DUMMY_EXIT) {
        buf.appendIndent(indent);

        if (StatEdge.EdgeType.BREAK.equals(edge.getType())) {
          addDeletedGotoInstructionMapping(stat, tracer);
          buf.append(OutputMemberSerializer.keyword("break"));
        }
        else if (StatEdge.EdgeType.CONTINUE.equals(edge.getType())) {
          addDeletedGotoInstructionMapping(stat, tracer);
          buf.append(OutputMemberSerializer.keyword("continue"));
        }
  ...
  public static TextBuffer listToJava(List<? extends Exprent> lst, int indent, BytecodeMappingTracer tracer) {
    ...
    for (Exprent expr : lst) {
      ...
      if (expr.type == Exprent.EXPRENT_BYTECODE_MARKER) {
        buf.append(content);
