...
import me.f1nal.trinity.decompiler.modules.decompiler.exps.FunctionExprent;
import me.f1nal.trinity.decompiler.modules.decompiler.exps.IfExprent;
import me.f1nal.trinity.decompiler.modules.decompiler.stats.IfStatement;
import me.f1nal.trinity.decompiler.modules.decompiler.stats.RootStatement;
import me.f1nal.trinity.decompiler.modules.decompiler.stats.SequenceStatement;
...
public final class IfHelper {
        ...
        if (ifbranch.succs.get(1).value == rtnode.succs.get(1).value) {
          ...
          if (ifchild.getFirst().getExprents().isEmpty()) {
            ...
            if (ifbranch.edgetypes.get(0) == 1) { // target null
              ...
              StatEdge ifedge = new StatEdge(StatEdge.EdgeType.REGULAR, ifparent.getFirst(), ifinner);
              ifparent.getFirst().addSuccessor(ifedge);
              ifparent.setIfEdge(ifedge);
    ...
    if (rtnode.edgetypes.get(1) == 0) {
      ...
      if (elsebranch.succs.size() == 2) {
          ...
          if (secondif.getFirst().getExprents().isEmpty()) {
            ...
            for (StatEdge edge : firstif.getAllPredecessorEdges()) {
              if (!firstif.containsStatementStrict(edge.getSource())) {
                ...
                edge.getSource().changeEdgeNode(StatEdge.EdgeDirection.FORWARD, edge, secondif);
        ...
        if (elsebranch.succs.get(0).value == rtnode.succs.get(0).value) {
          ...
          StatEdge newifedge = new StatEdge(StatEdge.EdgeType.REGULAR, firstif.getFirst(), second);
  ...
  private static IfNode buildGraph(IfStatement stat, boolean stsingle) {
    ...
    if (stsingle || edge.getType() != StatEdge.EdgeType.REGULAR) {
  ...
  private static boolean reorderIf(IfStatement ifstat) {
    ...
    if (ifstat.getIfstat() == null) {
      ...
      ifdirect = ifstat.getIfEdge().getType() == StatEdge.EdgeType.FINALLY_EXIT ||
      ...
      ifdirect = !lstSuccs.isEmpty() && lstSuccs.get(0).getType() == StatEdge.EdgeType.FINALLY_EXIT ||
    ...
    elsedirect = !last.getAllSuccessorEdges().isEmpty() && last.getAllSuccessorEdges().get(0).getType() == StatEdge.EdgeType.FINALLY_EXIT ||
    ...
    if ((ifdirect || ifdirectpath) && (elsedirect || elsedirectpath) && !noifstat && !noelsestat) {  // if - then - else
      ...
      StatEdge elseedge = new StatEdge(StatEdge.EdgeType.REGULAR, ifstat.getFirst(), stelse);
      ...
      if (noelsestat) {
        ...
        if (noifstat) {
          ...
          ifstat.addSuccessor(new StatEdge(StatEdge.EdgeType.REGULAR, ifstat, ifbranch));
        }
      }
      else {
        ...
        StatEdge newifedge = new StatEdge(StatEdge.EdgeType.REGULAR, ifstat.getFirst(), stelse);
