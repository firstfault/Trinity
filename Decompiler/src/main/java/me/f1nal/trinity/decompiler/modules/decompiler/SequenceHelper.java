...
public final class SequenceHelper {
        ...
        if (st.type == StatementType.SEQUENCE) {
          ...
          if (i == lst.size() - 1 || isSequenceDisbandable(st, lst.get(i + 1))) {
            ...
            for (StatEdge edge : st.getAllPredecessorEdges()) {
              ...
              edge.getSource().changeEdgeNode(StatEdge.EdgeDirection.FORWARD, edge, first);
            ...
            if (last.getAllSuccessorEdges().isEmpty() && i < lst.size() - 1) {
              last.addSuccessor(new StatEdge(StatEdge.EdgeType.REGULAR, last, lst.get(i + 1)));
            }
            else {
  ...
  private static boolean isSequenceDisbandable(Statement block, Statement next) {
    ...
    for (StatEdge edge : next.getPredecessorEdges(StatEdge.EdgeType.BREAK)) {
      ...
      for (Statement st : sequence.getStats()) {
        ...
        if (st.getExprents() != null && st.getExprents().isEmpty()) {
          ...
          if (st.getAllSuccessorEdges().isEmpty()) {
            List<StatEdge> lstBreaks = st.getPredecessorEdges(StatEdge.EdgeType.BREAK);
            ...
            if (sucedge.getType() != StatEdge.EdgeType.FINALLY_EXIT) {
              st.removeSuccessor(sucedge);

              for (StatEdge edge : st.getAllPredecessorEdges()) {
                if (sucedge.getType() != StatEdge.EdgeType.REGULAR) {
                  edge.getSource().changeEdgeType(StatEdge.EdgeDirection.FORWARD, edge, sucedge.getType());
                }

                st.removePredecessor(edge);
                edge.getSource().changeEdgeNode(StatEdge.EdgeDirection.FORWARD, edge, sucedge.getDestination());
