...
public final class IfStatement extends Statement {
  ...
  private IfStatement(Statement head, int regedges, Statement postst) {
    ...
    List<StatEdge> lstHeadSuccs = head.getSuccessorEdges(StatEdge.EdgeType.DIRECT_ALL);
    ...
    switch (regedges) {
      ...
      case 1 -> {
        ...
        if (edgeif.getType() != StatEdge.EdgeType.REGULAR) {
      ...
      case 2 -> {
        ...
        List<StatEdge> lstSucc = ifstat.getSuccessorEdges(StatEdge.EdgeType.REGULAR);
        List<StatEdge> lstSucc1 = elsestat.getSuccessorEdges(StatEdge.EdgeType.REGULAR);

        if (ifstat.getPredecessorEdges(StatEdge.EdgeType.REGULAR).size() > 1 || lstSucc.size() > 1) {
          post = ifstat;
        }
        else if (elsestat.getPredecessorEdges(StatEdge.EdgeType.REGULAR).size() > 1 || lstSucc1.size() > 1) {
          post = elsestat;
        }
        else {
          if (lstSucc.size() == 0) {
  ...
  public static Statement isHead(Statement head) {
    ...
    if (head.type == StatementType.BASIC_BLOCK && head.getLastBasicType() == StatementType.IF) {
      int regsize = head.getSuccessorEdges(StatEdge.EdgeType.REGULAR).size();
    ...
    if (ifstat == null) {
      ...
      if (ifedge.explicit) {
        ...
        if (ifedge.getType() == StatEdge.EdgeType.BREAK) {
          // break
          buf.appendIndent(indent + 1).append(OutputMemberSerializer.keyword("break"));
        }
        else {
          // continue
          buf.appendIndent(indent + 1).append(OutputMemberSerializer.keyword("continue"));
        }
  ...
  public void replaceStatement(Statement oldstat, Statement newstat) {
    ...
    List<StatEdge> lstSuccs = first.getSuccessorEdges(StatEdge.EdgeType.DIRECT_ALL);
