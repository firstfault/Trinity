...
import me.f1nal.trinity.decompiler.code.SwitchInstruction;
...
import me.f1nal.trinity.decompiler.modules.decompiler.exps.FieldExprent;
import me.f1nal.trinity.decompiler.modules.decompiler.exps.SwitchExprent;
...
public final class SwitchStatement extends Statement {
  ...
  private SwitchStatement(@NotNull Statement head, @Nullable Statement postStatement) {
    ...
    // find post node
    Set<Statement> regularSuccessors = new HashSet<>(head.getNeighbours(StatEdge.EdgeType.REGULAR, StatEdge.EdgeDirection.FORWARD));
    // cluster nodes
    if (postStatement != null) {
      post = postStatement;
      regularSuccessors.remove(post);
    }
    defaultEdge = head.getSuccessorEdges(StatEdge.EdgeType.DIRECT_ALL).get(0);
  ...
  public void initSimpleCopy() {
    first = stats.get(0);
    defaultEdge = first.getSuccessorEdges(StatEdge.EdgeType.DIRECT_ALL).get(0);
    sortEdgesAndNodes();
  }

  public void sortEdgesAndNodes() {
    Map<StatEdge, Integer> edgeIndicesMapping = new HashMap<>();
    List<StatEdge> firstSuccessors = first.getSuccessorEdges(StatEdge.EdgeType.DIRECT_ALL);
  ...
  private void collectExitEdgesIndices(@NotNull Map<StatEdge, Integer> edgeIndicesMapping,
                                       @NotNull List<@Nullable Statement> nodes,
                                       @NotNull List<List<Integer>> edgeIndices) {
    List<StatEdge> firstExitEdges = first.getSuccessorEdges(StatEdge.EdgeType.BREAK.unite(StatEdge.EdgeType.CONTINUE));
  ...
  private void sortEdges(List<@Nullable Statement> nodes, @NotNull List<List<Integer>> edgeIndices) {
    ...
    for (int index = 0; index < nodes.size(); index++) {
      ...
      HashSet<Statement> nodePredecessors = new HashSet<>(node.getNeighbours(StatEdge.EdgeType.REGULAR, StatEdge.EdgeDirection.BACKWARD));
