...
import me.f1nal.trinity.decompiler.modules.decompiler.sforms.DirectNode.DirectNodeType;
import me.f1nal.trinity.decompiler.modules.decompiler.stats.*;
import me.f1nal.trinity.decompiler.modules.decompiler.stats.*;
import me.f1nal.trinity.decompiler.modules.decompiler.stats.DoStatement.LoopType;
...
public class FlattenStatementsHelper {
        ...
        switch (stat.type) {
          case BASIC_BLOCK -> {
            ...
            if (tailExprentList != null) {
              ...
              listEdges.add(new Edge(node.id, -stat.id, StatEdge.EdgeType.REGULAR));
          ...
          case CATCH_ALL, TRY_CATCH -> {
            ...
            for (Statement st : stat.getStats()) {
              listEdges.add(new Edge(firstnd.id, st.id, StatEdge.EdgeType.REGULAR));
              ...
              if (stat.type == StatementType.CATCH_ALL && ((CatchAllStatement)stat).isFinally()) {
                ...
                if (st == stat.getFirst()) { // catch head
                  ...
                  stack.add(new StackEntry((CatchAllStatement)stat, Boolean.TRUE, StatEdge.EdgeType.BREAK,
          ...
          case DO -> {
            ...
            lstSuccEdges.add(stat.getSuccessorEdges(StatEdge.EdgeType.DIRECT_ALL).get(0));  // exactly one edge
            ...
            switch (loopType) {
              case WHILE, DO_WHILE -> {
                ...
                listEdges.add(new Edge(node.id, stat.getFirst().id, StatEdge.EdgeType.REGULAR));
                ...
                if (loopType == LoopType.WHILE) {
                  ...
                  for (Edge edge : listEdges) {
                    if (edge.statid.equals(stat.id) && edge.edgetype == StatEdge.EdgeType.CONTINUE) {
                      found = true;
                      break;
                    }
                  }
                  if (!found) {
                    listEdges.add(new Edge(nd.id, stat.id, StatEdge.EdgeType.CONTINUE));
                  }
                }
                sourcenode = node;
              }
              case FOR -> {
                ...
                listEdges.add(new Edge(nodecond.id, stat.getFirst().id, StatEdge.EdgeType.REGULAR));
                listEdges.add(new Edge(nodeinit.id, -stat.id, StatEdge.EdgeType.REGULAR));
                listEdges.add(new Edge(nodeinc.id, -stat.id, StatEdge.EdgeType.REGULAR));
          ...
          while (true) {
            ...
            if (entry == null) {
              ...
              if (entry.state) { // finally handler statement
                ...
                if (!catchall.containsStatementStrict(destination)) {
                  saveEdge(sourcenode, catchall.getHandler(), StatEdge.EdgeType.REGULAR, isFinallyExit ? finallyShortRangeSource : null,
  ...
  private void setEdges() {
    ...
    for (Edge edge : listEdges) {
      ...
      DirectNode dest = graph.nodes.getWithKey(mapDestinationNodes.get(statid)[edge.edgetype == StatEdge.EdgeType.CONTINUE ? 1 : 0]);
