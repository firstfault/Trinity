...
public final class CatchAllStatement extends Statement {
  ...
  public static Statement isHead(Statement head) {
    ...
    for (StatEdge edge : head.getSuccessorEdges(StatEdge.EdgeType.EXCEPTION)) {
      ...
      if (edge.getExceptions() == null && exc.getLastBasicType() == StatementType.GENERAL && setHandlers.contains(exc)) {
        List<StatEdge> lstSuccs = exc.getSuccessorEdges(StatEdge.EdgeType.DIRECT_ALL);
        if (lstSuccs.isEmpty() || lstSuccs.get(0).getType() != StatEdge.EdgeType.REGULAR) {
  ...
  public TextBuffer toJava(int indent, BytecodeMappingTracer tracer) {
    ...
    buf.append(isFinally ? " " + OutputMemberSerializer.keyword("finally") :
               " " + OutputMemberSerializer.keyword("catch") + " (" + vars.get(0).toJava(indent, tracer) + ")").append(" {").appendLineSeparator();
    tracer.incrementCurrentSourceLine();
