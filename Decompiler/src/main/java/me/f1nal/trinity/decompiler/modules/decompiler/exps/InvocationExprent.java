...
import me.f1nal.trinity.decompiler.modules.decompiler.ClasspathHelper;
  ...
  public TextBuffer toJava(int indent, BytecodeMappingTracer tracer) {
    ...
    if (isStatic) {
      ...
      if (node == null || !className.equals(node.classStruct.qualifiedName)) {
        buf.append(OutputMemberSerializer.tag(DecompilerContext.getImportCollector().getNestedNameInClassContext(ExprProcessor.buildJavaClassName(className)), l -> new ClassOutputMember(l, className)));
      }
    }
    else {
    ...
    switch (funcType) {
      ...
      case TYPE_INIT -> {
        ...
        if (super_qualifier != null) {
          ...
          buf.append(OutputMemberSerializer.tag("this", outputMemberFunction) + "(");
        }
        else if (instance != null) {
          buf.append(instance.toJava(indent, tracer)).append("." + OutputMemberSerializer.tag("<init:x1>", outputMemberFunction) + "(");
        }
        else {
