...
public abstract class Input<N> implements AccessFlagsMaskProvider, RenameHandler, IXrefBuilderProvider, IDisplayNameProvider {
    ...
    public void populatePopup(PopupItemBuilder builder) {
        ...
        builder.menu("Bytecode Name", names -> {
            names.disabled(() -> getDisplayName().getType() == RenameType.NONE,
                    enabled -> enabled.menuItem("Apply Display Name...",
                            () -> IdentityRefactorController.applyDisplayName(this)));
            names.menuItem("Rename Identity...",
                    () -> IdentityRefactorController.promptForIdentityName(this));
        });
        ...
        builder.disabled(() -> displayName.getType() == RenameType.NONE, (items) -> {
            items.menuItem("Revert Name", () -> {
                renameFully(Main.getTrinity().getRemapper(), displayName.getOriginalName());
