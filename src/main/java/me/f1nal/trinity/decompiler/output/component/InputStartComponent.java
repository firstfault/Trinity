...
public class InputStartComponent extends RawTextComponent {
    ...
    public void handleAfterDrawing() {
        ...
        ImVec2 rectMin = ImGui.getItemRectMin().plus(addX, 0.F);
        ImFont font = ImGui.getFont();
        String text = this.xrefs.size() + " usage" + (this.xrefs.size() == 1 ? "" : "s");
        ImVec2 textSize = font.calcTextSizeA(13.F, Float.MAX_VALUE, -1.F, text);
        ...
        ImGui.getWindowDrawList().addText(font, 13.F, rectMin.x, rectMin.y - 1.F, hovered ? CodeColorScheme.TEXT : CodeColorScheme.DISABLED, text);
        if (hovered) {
