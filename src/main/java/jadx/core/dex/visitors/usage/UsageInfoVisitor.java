...
/**
 * Trinity patch for JADX 1.5.6: scan independent classes concurrently while preserving
 * the upstream apply phase and deterministic sorted usage lists.
...
public class UsageInfoVisitor extends AbstractVisitor {
    ...
    public void init(RootNode root) {
        ...
        if (usageInfoData != null) {
            try {
                ...
                // Rebuild below if cached data cannot be applied.
            }
        }
    ...
    private static IUsageInfoData buildUsageData(RootNode root) {
        ...
        try {
            pool.submit(() -> root.getClasses().parallelStream()
                    .forEach(cls -> processClass(cls, usageInfo))).get();
        } catch (InterruptedException exception) {
            Thread.currentThread().interrupt();
            throw new JadxRuntimeException("Usage scan interrupted", exception);
        } catch (ExecutionException exception) {
            throw new JadxRuntimeException("Parallel usage scan failed", exception.getCause());
        } finally {
            pool.shutdown();
        }
        return usageInfo;
    ...
    private static void processMethod(MethodNode mth, UsageInfo usageInfo) {
        ...
        try {
            ...
            mth.addError("Dependency scan failed", exception);
        }
    }
        ...
        codeReader.visitInstructions(insnData -> {
            try {
                ...
                throw new JadxRuntimeException(
                        "Usage info collection failed with error: " + exception.getMessage()
                                + " at insn: " + insnData,
                        exception);
            }
        });
    ...
    private static void processInsn(RootNode root, MethodNode mth, InsnData insnData,
                                    UsageInfo usageInfo) {
        ...
        switch (insnData.getIndexType()) {
            ...
            case FIELD_REF:
                insnData.decode();
                FieldNode fieldNode = root.resolveField(
                        FieldInfo.fromRef(root, insnData.getIndexAsField()));
                if (fieldNode != null) {
    ...
    private static void processAnnotationAttr(ICodeNode node, AnnotationsAttr annAttr,
                                              UsageInfo usageInfo) {
    ...
    private static void processAnnotationValue(ICodeNode node, EncodedValue value,
                                               UsageInfo usageInfo) {
        ...
        switch (value.getType()) {
            ...
            case ENCODED_FIELD:
                if (obj instanceof IFieldRef) {
                    ...
                    throw new JadxRuntimeException(
                            "Unexpected field type class: " + value.getClass());
                }
                break;
            case ENCODED_ARRAY:
    ...
    public static void replaceMethodUsage(MethodNode mergeIntoMth, MethodNode sourceMth) {
        List<MethodNode> mergedUsage = ListUtils.distinctMergeSortedLists(
                mergeIntoMth.getUseIn(), sourceMth.getUseIn());
